...
from lisa.tools import Cat, Chrony, Dmesg, Hwclock, Lscpu, Ntp, Ntpstat, Servicectl
    ...
    def timesync_ntp(self, node: Node) -> None:
        if isinstance(node.os, Redhat) and node.os.release_version >= "8.0.0":
            ...
            raise SkippedException(
                f"The distro {node.os.name} {node.os.release_version} doesn't support "
                "ntp, because the ntp package is no longer supported and it is "
                "implemented by the chronyd (a daemon that runs in user-space) which "
                "is provided in the chrony package."
            )
        ntp = node.tools[Ntp]
        hwclock = node.tools[Hwclock]
        servicectl = node.tools[Servicectl]
        servicectl = servicectl.initialize_tool()
        # 1. Stop systemd-timesyncd if this service exists.
        servicectl.stop_service("systemd-timesyncd")  # type: ignore
        # 2. Set rtc clock to system time.
