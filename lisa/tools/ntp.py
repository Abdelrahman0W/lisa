...
from .servicectl import Servicectl
...
class Ntp(Tool):
    ...
    def restart(self) -> None:
        if isinstance(self.node.os, Debian) or (
            isinstance(self.node.os, Suse) and self.node.os.release_version <= "11.0.0"
        ):
            service_name = "ntp"
        elif isinstance(self.node.os, Redhat) or (
            isinstance(self.node.os, Suse) and self.node.os.release_version >= "12.0.0"
        ):
            service_name = "ntpd"
        else:
            posix_os: Posix = cast(Posix, self.node.os)
            raise LisaException(
                ...
                f"{posix_os.name} {posix_os.release_version}"
            )
        servicectl = self.node.tools[Servicectl]
        servicectl = servicectl.initialize_tool()
        servicectl.restart_service(service_name)  # type: ignore
